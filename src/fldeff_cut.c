...
bool8 SetUpFieldMove_Cut(void)
{
    ...
    else
    {
        ...
        {
            y = i - 1 + (gPlayerFacingPosition.y >> 4);
            for (j = 0; j < CUT_NORMAL_SIDE; j++)
            {
                x = j - 1 + (gPlayerFacingPosition.x >> 4);
        ...
        else
        {
            ...
            for (i = 0; i < 16; i++)
            {
                x = (gPlayerFacingPosition.x >> 4) + sHyperCutStruct[i].x;
                y = (gPlayerFacingPosition.y >> 4) + sHyperCutStruct[i].y;
                tileCuttable = TRUE;
...
bool8 FldEff_CutGrass(void)
{
    ...
    for (i = 0; i < CUT_HYPER_AREA; i++)
    {
        if (sHyperCutTiles[i] == TRUE)
        {
            ...
            x = xAdd + (gPlayerFacingPosition.x >> 4);
            y = yAdd + (gPlayerFacingPosition.y >> 4);
